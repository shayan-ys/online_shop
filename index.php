<?php require 'begin.php'; ?>
<?php include 'header.php'; ?>

				<section id="one">
					<div class="container">
						<div class="row">

							<div class="col-xs-12 col-sm-12 col-md-12">
								<div class="row">
									<div class="wr">
										<div class="slider-one">
											<div id="owl-demo-slider" class="owl-carousel owl-theme">

												<div class="item">
													<a href="http://orchardaz.com/wp-content/uploads/2015/04/refurb-apple-laptop-new2.jpg" target="_blank">

														<img src="http://orchardaz.com/wp-content/uploads/2015/04/refurb-apple-laptop-new2.jpg" width="870" height="330" alt="متفاوت باشید و متفاوت بمانید " title="متفاوت باشید و متفاوت بمانید " />

													</a>
												</div>

												<div class="item">
													<a href="http://dailycapital.pk/wp-content/uploads/2015/02/Apple-Macbook-Air-laptop.jpg" target="_blank">

														<img src="http://dailycapital.pk/wp-content/uploads/2015/02/Apple-Macbook-Air-laptop.jpg" width="870" height="330" alt="lenovo5170" title="lenovo5170" />

													</a>
												</div>

												<div class="item">
													<a href="http://g04.s.alicdn.com/kf/HT1eWqNFOJaXXagOFbXO/1903038/HT1eWqNFOJaXXagOFbXO.jpg" target="_blank">

														<img src="http://g04.s.alicdn.com/kf/HT1eWqNFOJaXXagOFbXO/1903038/HT1eWqNFOJaXXagOFbXO.jpg" width="870" height="330" alt="حراجی سری جی " title="حراجی سری جی " />

													</a>
												</div>



											</div>
										</div>



										<div id="content" class="neg">
											<article id="amazingoffer" class="dkbox mrg-bottom">
												<div class="slides center">
													<a style="display: none;" href="pages/p506.php" title="لنوو زد 5170 گرافیک 4 گیگابایت" class="slideItem">
														<img class="incredible" src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="607" height="300" alt="لنوو زد 5170 گرافیک 4 گیگابایت" title="لنوو زد 5170 گرافیک 4 گیگابایت" />
												</a>
												<a style="display: none;" href="pages/book100.php" title="قلندران پيژامه پوش" class="slideItem">
													<img class="incredible" src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="607" height="300" alt="قلندران پيژامه پوش" title="قلندران پيژامه پوش" />
											</a>

											<a style="display: none;" href="pages/book101.php" title="ناز و نياز " class="slideItem">
												<img class="incredible" src="http://www.ofoqco.com/images/books/344680.jpg" width="607" height="300" alt="ناز و نياز " title="ناز و نياز " />
										</a>

										<a style="display: none;" href="pages/book102.php" title="امپراتور کوتوله ی سرزمین لی لی پوت " class="slideItem">
											<img class="incredible" src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="607" height="300" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
									</a>


								</div>

								<div class="ff">
									<ul class="tabs">
										<li><a href="pages/p506.php" class="tabItem"><span>لنوو زد 5170 گرافیک 4 گیگابایت</span></a></li>
										<li><a href="pages/book100.php" class="tabItem"><span>قلندران پيژامه پوش</span></a></li>
										<li><a href="pages/book101.php" class="tabItem"><span>ناز و نياز </span></a></li>
										<li><a href="pages/book102.php" class="tabItem"><span>امپراتور کوتوله ی سرزمین لی لی پوت </span></a></li>
									</ul>
								</div>
							</article>
						</div>


					</div>
				</div>
			</div>

</div>
</div>
</section>

<section id="two">
	<div class="container">
		<div class="row">


			<div class="col-xs-12 col-sm-4 col-lg-3">
				<div class="row">
					<div class="wr">
						<div class="box">
							<div class="title bo1">
								<h2>پرفروش ترین محصولات</h2>
							</div>
							<div class="list">
								<ul>

									<li>
										<div class="wall">
											<a href="pages/book102.php">
												<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="80" height="80" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
											</a>
										</div>
										<div class="text">
											<h3>
												<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
											</h3>
											<span class="price">۶,۰۰۰ تومانومان</span>
											<span class="rating score-callback" data-score="0"></span>
											<span class="existent wew1">موجود</span>
										</div>
									</li>

									<li>
										<div class="wall">
											<a href="p506.php">
												<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="80" height="80" alt="Lenovo Z5170 i7 RAM 16GB 1T " title="Lenovo Z5170 i7 RAM 16GB 1T " />
											</a>
										</div>
										<div class="text">
											<h3>
												<a href="p506.php">Lenovo Z5170 i7 RAM 16GB 1T </a>
											</h3>
											<span class="price">۲,۸۷۵,۰۰۰ تومان</span>
											<span class="rating score-callback" data-score="0"></span>
											<span class="existent wew1">موجود</span>
										</div>
									</li>

									<li>
										<div class="wall">
											<a href="pages/book101.php">
												<img src="http://www.ofoqco.com/images/books/344680.jpg" width="80" height="80" alt="ناز و نياز " title="ناز و نياز " />
											</a>
										</div>
										<div class="text">
											<h3>
												<a href="pages/book101.php">ناز و نياز </a>
											</h3>
											<span class="price">۱,۳۹۵,۰۰۰ تومان</span>
											<span class="rating score-callback" data-score="0"></span>
											<span class="existent wew1">موجود</span>
										</div>
									</li>
								</ul>
							</div>
						</div>
						<div class="box">
							<div class="title bo4">
								<h2>پیگیری سفارش</h2>
							</div>
							<div class="boxpam">
								<p>برای رهگیری سفارش ، لطفا کد رهگیری 16 رقمی سفارش خود را وارد کنید</p>
								<form id="formtrack" class="not_ajax ocar" action="" >
									<div class="order_state">
										<input id="" type="text" name="track" class=""/>
										<input value="پیگیری" type="submit" />
									</div>
								</form>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-xs-12 col-sm-8 col-lg-9">
				<div class="row">
					<div class="wl">
						<div class="box">
							<div class="title bo2">
								<h2>جدیدترین محصولات</h2>
							</div>
							<div class="slider-product">
								<div id="owl-demo2">
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>



						<div class="box">
							<div class="title bo3">
								<h2>محبوب ترین محصولات</h2>
							</div>
							<div class="slider-product">
								<div id="owl-demo3">
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>



						<div class="box lb">
							<div class="title bo4">
								<h2>پیشنهاد ما</h2>
							</div>
							<div class="slider-product">
								<div id="owl-demo4">
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book102.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/345267.jpg" width="131" height="131" alt="امپراتور کوتوله ی سرزمین لی لی پوت " title="امپراتور کوتوله ی سرزمین لی لی پوت " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book102.php">امپراتور کوتوله ی سرزمین لی لی پوت </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۶,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/p506.php">
													<img src="http://static.bamilo.com/p/asus-4877-6933731-1-product.jpg" width="131" height="131" alt="Lenovo Z5170 i7 RAM 16GB 1T" title="Lenovo Z5170 i7 RAM 16GB 1T" />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/p506.php">Lenovo Z5170 i7 RAM 16GB 1T</a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="4"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۸۷۵,۰۰۰</span> تومان</div>
											</div>
										</div>
									</div>

									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book101.php">
													<img src="http://www.ofoqco.com/images/books/344680.jpg" width="131" height="131" alt="ناز و نياز " title="ناز و نياز " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book101.php">ناز و نياز </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۲,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
									<div class="item">
										<div class="product">
											<div class="wall">
												<a href="pages/book100.php">
													<img src="http://www.ofoqco.com/images/books/Thumb/344653.jpg" width="131" height="131" alt=" قلندران پيژامه پوش " title=" قلندران پيژامه پوش " />
												</a>
											</div>
											<div class="info">
												<h3>
													<a href="pages/book100.php"> قلندران پيژامه پوش </a>
												</h3>
												<ul>
													<li class="wew1">موجود</li>
													<li>
														<span class="rating score-callback" data-score="0"></span>
													</li>
												</ul>
												<div class="price"><span>۳,۵۰۰</span> تومان</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>


					</div>
				</div>
			</div>

		</div>
	</div>
</section>

<script>
$("#formtrack").on('submit', function() {
	action = $(this).attr('action');
	value = $('#trackOrder').val();
	valuelength = value.length;
	if (valuelength > 2) {
		$(this).attr('ajax_method', 'get');
		window.location = action + '&order&track=' + value;
	} else {
		$('body').ham3dModalbox({html: '<br /><a href="javascript:void(0);" onclick="$(\'javascript:void(0)modalbox-wrap\').fadeOut(); return false;"><div class="add_popup"><div class="add_popup_content"><span>ساختار کد پیگیری شما اشتباه می باشد !</span></div></div></a>', auto: 'ok'}).trigger('click');
		$('#modalbox-wrap').fadeOut(5000);
	}
	return false;
});
</script>
<?php include 'footer.php'; ?>



<!------------------------------------------------------------------------------------------>

<?php include 'end.php'; ?>
